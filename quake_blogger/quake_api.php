...
class Quake_api {
	...
	*/
	...
	public function format_api_results_and_post($response) {
		...
		foreach($decoded_response->features as $key => $event) {
			$event_details .= '<li>'.
			                    ...
			                    $this->format_date_time($event->properties->time).
			                  '</li>';
			array_push($event_array, $event_details);
		}
		//we only want the first 3 items as excerpt
		$post_excerpt  = '<ul>'.implode(' ', array_slice($event_array, 0, 3)).'</ul>';
		$post_content  = '<ul>'.$event_details.'</ul>';
	...
	public function generate_post_title($decoded_response) {
		$offset_date  = $this->format_date_time($this->offset_date, false);
		$current_date = $this->format_date_time($this->current_date, false);
		$postTitle    = $decoded_response->metadata->title.": $offset_date - $current_date (".$decoded_response->metadata->count." events logged)";

		return $postTitle;
	}

	public function format_date_time($date, $is_timestamp=true) {
		if ($is_timestamp)
			$date = date('c', $date/1000);
